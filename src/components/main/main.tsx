...
export function Main() {
	...
	const createLines = useCallback( (parentid: any) => {
		const defaultBody = {
			equipmentCosts: 0,
			estimatedProfit: 0,
	...
	const updateLines = useCallback((
		...
		update({ rID: Number(rID), bod: defaultBody })
	},[])
	const deleteLines = useCallback( (rID: string) => {
		delet(rID).unwrap()
	},[])
			...
			{data.map((dat: any) => {
				return (
					<React.Fragment key={dat.id}>
						...
						>
							<td
								style={{paddingLeft:nesting*10}} 
								className={`text-center items-center w-36 pt-2 `}
							>
